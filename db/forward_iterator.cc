...
void ForwardIterator::RenewIterators() {
  ...
  bool level_files_same = true;
  for (int32_t level = 1; level < vstorage_new->num_levels(); ++level) {
    const auto& level_files = vstorage_new->LevelFiles(level);
    const auto& level_files_old = vstorage->LevelFiles(level);
    level_files_same = true;
    if (level_files.size() != level_files_old.size()) {
      level_files_same = false;
    } else {
      for (uint32_t i = 0; i < level_files.size(); i++) {
        if (level_files[i] != level_files_old[i]) {
          level_files_same = false;
          break;
        }
      }
    ...
    if ((level_files.empty()) ||
      ...
      level_iters_[level-1] = new LevelIterator(cfd_, read_options_, level_files);
    }
  }
