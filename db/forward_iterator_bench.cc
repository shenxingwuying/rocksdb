...
struct StatsThread {
  ...
  void run() {
    ...
    auto tstart = steady_clock::now(), tlast = tstart;
    ...
    while (!done_.load()) {
      ...
      double elapsed = duration_cast<duration<double>>(now - tlast).count();
      ...
      fprintf(stderr,
              ...
              duration_cast<seconds>(now - tstart).count(), w,
              (w - wlast) / elapsed, r, (r - rlast) / elapsed,
